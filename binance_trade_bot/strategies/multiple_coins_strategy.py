...
class Strategy(AutoTrader):
    def scout(self):
        ...
        all_balances = self.manager.get_currency_balance_all()
        for coin in self.db.get_coins():
            current_coin_balance = self.manager.find_coin_balance_from(
                coin.symbol, all_balances
            )  # self.manager.get_currency_balance(coin.symbol)
